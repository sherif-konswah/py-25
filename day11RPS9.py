...
def rps(name='Player one'):
    ...
    def play_rps():
        ...
        print('************************************')
        rpc1 =int(input(f"{name} : Pick Your Type...\n 1 For Rock, \n 2 For Paper, \n 3 For Scissor:\n\n"))
        rpc2 =int(random.choice("123"))

        if(str(rpc1) not in ["1","2","3"]):
            print(f"😱😱 {name},You Must Enter Number From 1 To 3 Only!!!!")
            return play_rps()
        
        print(f"{name} Chose {str(RPS(rpc1)).replace("RPS.","")}  .")
        ...
        def decide_winner (rpc1,rpc2):
            ...
            if rpc1 == rpc2 :
                ...
                player_wins_count +=1
                return f"🎉 {name} , You Win >>>> Rock Win Scissor" , player_wins_count
        ...
        print(f"\n Game Count : {str(game_count)} \n Total Result  : \n {name}  >>>   {str(player_wins_count)}    VS    {str(com_wins_count)}    <<<   PC")
